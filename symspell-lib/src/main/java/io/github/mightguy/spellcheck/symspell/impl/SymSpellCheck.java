...
public class SymSpellCheck extends SpellChecker {
     ...
     */
        ...
        for (int i = 0; i < items.length; i++) {
            //Normal suggestions
            suggestions = lookup(items[i], Verbosity.ALL, maxEditDistance);

            String itemMetaphoneCode = doubleMetaphone.encode(items[i]);


            /***
             *
             * need to add reranking by context, bigram,trigram etc
             */

            //get bigrams of suggestions[k] with items[i+1], and bingrams of items[i-1] + suggestions[k], use freq to rerank

            Map<SuggestionItem, Double> suggestion2NgramWeightMap = new HashMap<>();

            if (items.length > 1) {

                for (int k = 0; k < suggestions.size(); k++) {

                    SuggestionItem suggestionItem = suggestions.get(k);

                    String suggestionMetaphoneCode = doubleMetaphone.encode(suggestionItem.getTerm());

                    suggestionItem.setMetaphoneCode(suggestionMetaphoneCode);
                    suggestionItem.setSameMetaphoneCode(suggestionMetaphoneCode.equals(itemMetaphoneCode));

                    String bigram1 = null;
                    String bigram2 = null;
                    Double bigram1Freq = 0d;
                    Double bigram2Freq = 0d;

                    String trigram1 = null;
                    String trigram2 = null;
                    String trigram3 = null;
                    Double trigram1Freq = 0d;
                    Double trigram2Freq = 0d;
                    Double trigram3Freq = 0d;

                    double unigramFreq = suggestionItem.getCount() + 1;

                    Double freq = null;

                    if (i < items.length - 1) {
                        bigram1 = suggestionItem.getTerm() + " " + items[i + 1];
                    }

                    if (i > 0) {
                        bigram2 = items[i - 1] + " " + suggestionItem.getTerm();
                    }

                    if (i < items.length -2 ) {
                        trigram1 = suggestionItem.getTerm() + " " + items[i + 1] + " " + items[i+2];
                    }

                    if (i < items.length - 1 && i > 0) {
                        trigram2 = items[i-1] + " " +  suggestionItem.getTerm() + " " + items[i + 1];
                    }

                    if ( i > 1) {
                        trigram3 = items[i-2] + " " +  items[i - 1] + " " + suggestionItem.getTerm() ;
                    }



                    if (bigram1 != null)
                        freq = dataHolder.getItemFrequencyBiGram(bigram1);
                    if (freq != null) {
                        bigram1Freq = freq;
                    }
                    ...
                    if (bigram2 != null) {
                        ...
                        freq = dataHolder.getItemFrequencyBiGram(bigram2);
                        if (freq != null) {
                            bigram2Freq = freq;
                        }
                    }
                    ...
                    if (trigram1 != null)
                        freq = dataHolder.getItemFrequencyTriGram(trigram1);
                    if (freq != null) {
                        trigram1Freq = freq;
                    }
                    ...
                    if (trigram2 != null) {
                        ...
                        freq = dataHolder.getItemFrequencyTriGram(trigram2);
                        if (freq != null) {
                            trigram2Freq = freq;
                        }
                    }
                    ...
                    if (trigram3 != null) {
                        ...
                        freq = dataHolder.getItemFrequencyTriGram(trigram3);
                        if (freq != null) {
                            trigram3Freq = freq;
                        }
                    }

                    double weight = (bigram1Freq + bigram2Freq) / unigramFreq + 1000 * ((trigram1Freq + trigram2Freq + trigram3Freq) / unigramFreq);
                    suggestionItem.setScore(weight);
                    suggestion2NgramWeightMap.put(suggestionItem, weight);


                }
                suggestion2NgramWeightMap.size();


                Map<SuggestionItem, Double> sortedSuggestion2NgramWeightMap = sortItemByWeightDesc(suggestion2NgramWeightMap);
                List<SuggestionItem> newList = new ArrayList<>();
                newList.addAll(sortedSuggestion2NgramWeightMap.keySet());
                suggestions = newList;
    ...
    private Map<SuggestionItem, Double> sortItemByWeightDesc(Map<SuggestionItem, Double> unsortedMap) {

        LinkedHashMap<SuggestionItem, Double> reverseSortedMap = new LinkedHashMap<>();
